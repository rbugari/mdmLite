...
export async function GET() {
  ...
  await query(
    ...
    [
      createId(),
      createId(),
      JSON.stringify({ kind: "mappings", count: result.rows.length, exportedAt }),
      identity.userId,
      "Exported mappings CSV",
    ],
  );
  ...
  return new Response(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${exportFilename("mdm-mappings")}"`,
